...
public class DriveOnStreet : Agent
{
    ...
    void RespawnAgent()
    {
        ...
        // _movementController.SetBrake(0);
        ...
        while(_posOccupied)
        {
            ...
            randomZ = Random.Range(-37, 37);
            Vector3 spawnPos = new Vector3(1.7f, 0.4f, randomZ);
            
            if(!clockwise)
            {
                spawnPos = new Vector3(-1.7f, 0.4f, randomZ);
            }
            var hitColliders = Physics.OverlapSphere(spawnPos, radius);
        ...
        if(clockwise)
        {
            this.transform.localPosition = new Vector3(1.7f, 1f, randomZ);
